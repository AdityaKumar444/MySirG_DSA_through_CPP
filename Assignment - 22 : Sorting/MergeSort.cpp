#include<bits/stdc++.h>
using namespace std ;
void mergeSorted(int *,int,int,int);                        // merge function to merge two sorted arrays
void MergeSort(int*,int,int);                               // MergeSort method to sort elements
...
void mergeSorted(int *arr, int start, int mid, int end)
{
    ...
    int *arr1 = new int[len1];                              // dynamically created first sub-array temporarily
    int *arr2 = new int[len2];                              // dynamically created second sub-array temporarily
...
int main () 
{
    int arr[10] = {75,29,83,42,16,90,56,34,20,71};
    MergeSort(arr,0,9);
    for(int i=0 ; i<10 ; i++)
        cout << arr[i] << " " ;
    cout << endl ;
